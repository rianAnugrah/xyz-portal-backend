...
export async function createArticle(fastify: FastifyInstance) {
  fastify.post<{
    ...
    try {
      ...
      if (!request.body.platform_id || isNaN(platformId) || platformId < 0 || platformId > 9) {
      ...
      if (articleId === undefined || articleId === null) {
        ...
        if (lastArticle && lastArticle.article_id !== null && lastArticle.article_id !== undefined) {
          ...
          if (sequence > 999) {
            throw new Error("Sequence exceeded maximum value (999) for this day");
          }
        }

        // Construct article_id as an integer: [platform_id][YY][MM][DD][sequence]
        articleId = Number(`${platformId}${year}${month}${day}${sequence.toString().padStart(3, "0")}`);
        if (articleId > 2147483647) {
          throw new Error("Generated article_id exceeds integer limit (2,147,483,647)");
        }
      }
